...
- **Batch 1** — write the report header (`subagent:`, `question:`,
  `status: partial`, `batch: 1`) plus the first N assumptions to the
  canonical file. Then append a `continuation_request:` block at the
  end of the file with the remaining assumptions count and next actions.
...
- **Batch 2+** — the main agent re-spawns you with
  ...
  parent provides, append the remaining assumptions, switch
...
`status`, `batch`, `findings`) and append assumptions across batches
