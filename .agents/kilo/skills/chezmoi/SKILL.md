...
- **`.encrypted_data/env_store/`:** Profile-scoped, age-encrypted shell snippets for exporting environment variables (tokens, API keys). File naming follows `<profile>_<name>.zsh.age`. The `dot_shell/helper/executable_runpriv.tmpl` script injects decrypted environment variables on-demand at process launch, avoiding persistent token exposure in the shell environment. The launched process also receives `SHELLX_VARS` (and the legacy `RUNPRIV_VARS` alias, same value) — a comma-separated list of the injected secret variable names. `shellx` (Python, replaces `runpriv`) writes age-encrypted JSONC exports here by default — see `.encrypted_data/env_store/README.md` for conventions. (Renamed from `tokens/` in `shellx 1.2.0`.)
